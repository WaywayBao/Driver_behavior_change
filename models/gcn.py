...
class GCN(nn.Module):
    def __init__(self, inputs, time_steps=90, pretrained=True, partialConv=False, fusion='avg'):
        ...
        # build backbones
        if self.partialConv:
            self.backbone = pdresnet50(pretrained=True)
    ...
    def forward(self, camera_inputs, trackers, device, dist_mask, mask=None):
        ...
        if self.partialConv:
            camera_inputs = self.backbone.features(
                camera_inputs.view(-1, c, w, h), mask.view(-1, c, w, h))  # camera_inputs :(bs,t,c,w,h)
        else:
            camera_inputs = self.backbone.features(
        ...
        # camera_inputs = camera_inputs.view(batch_size, t, 1536, 8, 8)  # BxTxCxWxH
